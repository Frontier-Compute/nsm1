...
COPY --from=builder /app/target/release/nsm1 /usr/local/bin/
COPY --from=builder /app/target/release/anchor_root /usr/local/bin/
COPY --from=builder /app/target/release/BUILD_INFO /usr/local/share/nsm1/
RUN mkdir -p /data
VOLUME /data
EXPOSE 3080
CMD ["nsm1"]

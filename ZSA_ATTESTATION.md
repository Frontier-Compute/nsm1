...
ZSA_REFERENCE_NOTE    = BLAKE2b_32(0x12 || len(issuer) || issuer || assetDescHash || len(txid) || txid || output_index_be)
ZSA_ISSUE             = BLAKE2b_32(0x13 || len(issuer) || issuer || assetDescHash || amount_be || len(txid) || txid || issue_action_index_be)
ZSA_FINALIZE          = BLAKE2b_32(0x14 || len(issuer) || issuer || assetDescHash || len(txid) || txid)
ZSA_ZERO_ISSUE_REVOKE = BLAKE2b_32(0x15 || len(issuer) || issuer || assetDescHash || len(txid) || txid || reason_code_be)
ZSA_BURN              = BLAKE2b_32(0x16 || asset_base || amount_be || len(txid) || txid)
...
ZSA_REDEEM_REQUEST    = BLAKE2b_32(0x1A || len(issuer) || issuer || assetDescHash || len(burn_txid) || burn_txid || amount_be || destination_hash)
ZSA_REDEEM_COMPLETE   = BLAKE2b_32(0x1B || len(issuer) || issuer || assetDescHash || len(burn_txid) || burn_txid || settlement_txid_hash)
